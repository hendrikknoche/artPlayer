...
gg2 <- gg2[-(1:7), ]
gg2 <- subset(gg2, variable =="Avg.HR")
...
gg3 <- gg3[-(1:7), ]
gg3 <- subset(gg3, variable =="Avg.HR")
...
gg2$value <- gg2$value-gg$value
gg3$value <- gg3$value-gg$value
...
sumCollected <- rbind(ggSum2, ggSum3)

pdf(file="ci2PlotHR.pdf",width=5,height=2.8)
ggplot(sumCollected, aes(episode, value, colour=episode)) + theme_bw() + #hele plottet, s?rg for at bruge de rigtige variabler
  ...
  ylab("Average HR ") + xlab("Episode") + theme(legend.position="none") +
