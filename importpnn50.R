...
gg3 <- gg3[-(1:7), ]
gg3 <- subset(gg3, variable =="pNN50")
gg3 <- subset(gg3, value != 0)
...
pdf(file="ci2PlotpNN50.pdf",width=5,height=2.8)
ggplot(sumCollected, aes(episode, value, colour=episode)) + theme_bw() + #hele plottet, s?rg for at bruge de rigtige variabler
  ...
  ylab("pNN50 power ") + xlab("Episode") + theme(legend.position="none") +
